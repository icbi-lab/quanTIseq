...
source("/opt/quantiseq/deconvolution/DOCKER_codes.R")
...
if (signame %in% listsig) {
  ...
  sig.mat.file<-file.path("/opt/quantiseq/deconvolution/",
                          paste0(signame, "_signature.txt"))
  
  mRNA.file<-file.path("/opt/quantiseq/deconvolution/",
                       paste0(signame, "_mRNA_scaling.txt"))
  
  fileab<-file.path("/opt/quantiseq/deconvolution/",
