...
preprocessing <- function(inputfile, outputpath, threads, phred, adapters, adapterSeed, palindromeClip, simpleClip, trimLead, trimTrail, minlen, crop)
  ...
  # Run Trimmomatic for each line of the input file:
    ...
    if (input[i,3] == "None"){
      libtype = "SE"
      files = paste0("/opt/quantiseq/Input/", basename(input[i,2]), " ", outputpath, basename(input[i,2]))
    }
      ...
      files = paste0("/opt/quantiseq/Input/", basename(input[i,2])," /opt/quantiseq/Input/", basename(input[i,3])," ", out1," ", out1, "_nopair ", out2," ", out2, "_nopair")
    }
    
    system(paste0("java -jar /opt/quantiseq/trimmomatic/trimmomatic-0.36.jar ",libtype," -threads ",threads," -phred", phred," ",files," ILLUMINACLIP:",adapters,":",adapterSeed,
